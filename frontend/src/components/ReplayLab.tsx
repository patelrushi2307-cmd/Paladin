...
export function ReplayLab({ sources, status, onStatus }: { sources: ReplaySource[]; status: ReplayStatus; onStatus: (status: ReplayStatus) => void }) {
  ...
  try {
    ...
    if (name === 'start' && !selectedSource) return;
    setBusy(true);
    try { onStatus(await replayAction(name, name === 'start' ? { source_type: selectedSource.source_type, source_path: selectedSource.path, mode, speed_multiplier: 1 } : undefined)); } finally { setBusy(false); }
  }
